...
case_id = ['trade_web_account_transfer_01']
TestData = GetTestCase.case_data(case_id)
re_data = regular(str(TestData))
print("re_data:",re_data)

#处理多个异常数据
list2=generateData(eval(re_data)[0],"pre_trade_web_account_transfer.yaml")

@allure.epic("交易平台接口")
@allure.feature("futures模块")
class TestTradeWebAccountTransfer:

    @allure.story("web发起划转")
    # @pytest.mark.parametrize('in_data', eval(re_data), ids=[i['detail'] for i in TestData])
    @pytest.mark.parametrize('in_data', list2, ids=[i['detail'] for i in list2])
    def test_trade_web_account_transfer_01(self, in_data, case_skip):
        """
        :param :
        :return:
        """
