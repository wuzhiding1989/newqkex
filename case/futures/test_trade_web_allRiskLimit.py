...
case_id = ['trade_web_allRiskLimit_01']
TestData = GetTestCase.case_data(case_id)
re_data = regular(str(TestData))
print("re_data:",re_data)
list2=generateData(eval(re_data)[0],"pre_trade_web_allRiskLimit.yaml")
@allure.epic("交易平台接口")
@allure.feature("futures模块")
class TestTradeWebAllRiskLimit:

    @allure.story("查看风险限额-与仓位挂单对应档位")
    # @pytest.mark.parametrize('in_data', eval(re_data), ids=[i['detail'] for i in TestData])
    @pytest.mark.parametrize('in_data', list2, ids=[i['detail'] for i in list2])
    def test_trade_web_allRiskLimit_01(self, in_data, case_skip):
        """
        :param :
        :return:
        """
