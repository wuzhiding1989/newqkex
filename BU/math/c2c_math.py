...
def mysql_fee_selec(user_id,legal_symbol,symbol):#通过数据库获取用户数据计算广告费率
    ...
    if status==0:
        ...
        if fee_rate==0:
            ...
            return fee_1
def otc_ratiofee(fee_rate,status,ratio,fee,account):#计算费率返佣的
...
if __name__ == '__main__':
    c=otc_ratiofee(fee_rate=0,status=1,ratio=3.020,fee=0.1,account=5000)
    #a=mysql_fee_selec(user_id=10122165,legal_symbol='usd',symbol='btc')
    print(c)